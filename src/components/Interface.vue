<template>
    ...
    <div class="site">
        <div class="information">
            ...
            <h3>
                Find out everything about the current status of your finances.
            </h3>
            <h4>
                View the performance of your stocks, your current portfolio and
        ...
        <div class="container" id="statisticsTable">
            ...
            >
                ...
                <tbody class="table-group-divider">
                    <tr>
                        <td>
                            {{ holdingsStatistics[0] }}
                            {{ holdingsStatistics[5] }}
                        </td>
                        <td>
                            {{ holdingsStatistics[1] }}
                            {{ holdingsStatistics[5] }}
                        </td>
                        <td>
                            {{ holdingsStatistics[2] }}
                            {{ holdingsStatistics[5] }}
                        </td>
                        <td>
                            {{ holdingsStatistics[3] }}
                            {{ holdingsStatistics[5] }}
                        </td>
                        <td>
                            {{ holdingsStatistics[4] }}
                            {{ holdingsStatistics[5] }}
                        </td>
                    </tr>
                </tbody>
        ...
        <div class="container" id="performanceTable">
            <div class="info-text">
                ...
                <h5>
                    Find out at a glance which stocks you are holding at the
                    moment.
                </h5>
            ...
            >
                ...
                <tbody class="table-group-divider">
                    <tr v-for="i in holdings" :key="i.holdingId.POSITION">
                        ...
                        <td>
                            {{ i.value.amount.toFixed(2) }}
                            {{ i.value.currencyId }}
                        </td>
                        <td>{{ i.quantity.toFixed(2) }}</td>
                        <td>
                            {{ i.quote.amount.toFixed(2) }}
                            {{ i.value.currencyId }}
                        </td>
                        <td
                            v-if="
                        ...
                        <td v-else>
                            {{
                                (i.value.amount / holdingsStatistics[4])
                                    .toFixed(4)
                                    .slice(2, 4)
                            }}.{{
                                (i.value.amount / holdingsStatistics[4])
                    ...
                    <tr class="table-success">
                        ...
                        <td class="fat">
                            Total Quantity: {{ totalQuantityArr[0] }}
                        </td>
                        <td></td>
                        <td></td>
                    </tr>
            ...
            >
                ...
                <tbody class="table-group-divider">
                    <tr
                        ...
                        :key="i.transactionId"
                        v-bind:class="
                            i.actualAmount.amount > 0
                                ? 'table-success'
                                : 'table-danger'
                        "
                    >
                        <td
                            v-if="i.transactionDetailedType.includes('Verkauf')"
                        >
                            {{
                        ...
                        <td
                            v-if="i.transactionDetailedType.includes('Einlage')"
                        >
                            {{
                                i.transactionDetailedType
                                    .split('_')
                        ...
                        <td
                            v-if="
                                i.transactionDetailedType.includes('Entnahme')
                            "
                        >
                            {{
                                i.transactionDetailedType
...
export default {
    ...
    methods: {
        ...
        async fetchValues() {
            ...
            for (let i = 0; i < responseJsonValue.holdings.length; i++) {
                totalQuantity += parseFloat(
                    responseJsonValue.holdings[i].quantity
                )
            }
        ...
        async fetchStatistics() {
            ...
            const beginValue =
                responseJsonStatistics.holdings[0].valueStatistic.beginValue
                    .amount
            const endValue =
                responseJsonStatistics.holdings[0].valueStatistic.endValue
                    .amount
            const minValue =
                responseJsonStatistics.holdings[0].valueStatistic.minValue
                    .amount
            const maxValue =
                responseJsonStatistics.holdings[0].valueStatistic.maxValue
                    .amount
            const averageValue =
                responseJsonStatistics.holdings[0].valueStatistic.averageValue
                    .amount
            const currencyStatistics =
                responseJsonStatistics.holdings[0].accruedInterestStatistic
                    .averageValue.currencyId
    ...
    mounted() {
        if (navigator.onLine == false) {
            alert(
                'You must be connected to the Internet to view your finances!'
            )
        }
